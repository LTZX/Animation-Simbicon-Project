...
void SimBiController::computeTorques(DynamicArray<ContactPoint> *cfs) {
    ...
    // -- following method is re-implemented by Zhiyi and Min --
    PoseController::newComputeTorques(cfs);
    ...
    // -- following method is re-implemented by Zhiyi and Min --
    newComputeHipTorques(qRootD, poseRS.getJointRelativeOrientation(swingHipIndex), stanceHipToSwingHipRatio);
...
// -- re-implemented by Zhiyi and Min --
void SimBiController::newComputeHipTorques(const Quaternion& rootTargetQ, const Quaternion& targetSwingHipQ, double stanceHipToSwingHipRatio)
{
    Vector3d rootTorque;
    Vector3d swingHipTorque;

    // From our understanding of the paper, the root here refers to the torso
    Quaternion  rootTargetQW; // desired orientation of the torso in world frame;

    // we choose to disable heading control, so that the torso will just maintain
    // the current target orientation
    rootTargetQW = characterFrame * rootTargetQ;
    ...
    // use the original implementation of computeTorque() to derive the net torque for the torso
    // in world coordinates as mentioned in the paper
    rootTorque = computePDTorque(root->getOrientation(), rootTargetQW, root->getAngularVelocity(), Vector3d(0, 0, 0), &rootControlParams);

    RigidBody* swingHip = character->getJoint(swingHipIndex)->getChild();

    // swingHipTorque is already calculated because of the call to PoseController::newComputeTorques()
    // directly fetch the stored results
    swingHipTorque = torques[swingHipIndex];

    // this torque is total torque of applying external forces
    // on two hips to need to be 
    // applied by the controller, such that it cancels
    // out two hip torques, while gives torso the net torque of qRootTorque
    Vector3d totalTorque;

    // totalTorque + t_A + t_B + rootTorque = 0
    for (int i = 0; i < jointCount; i++)
        if (character->getJoint(i)->getParent() == root)
            totalTorque -= torques[i];

    totalTorque -= rootTorque;

    // we did not consider predictive torque, as well as
    // the damping, since the paper did not cover related details.
    // In this way, we can examine the effect of removing these two parts on simultion.

    // assume the stance foot is in contact
    Vector3d stanceHipTorque = torques[stanceHipIndex];

    // calculate the torque for each hip according to the ratio of forces exerted on
    // two foot, stanceFoot -> stanceHip, swingFoot -> swingHip
    stanceHipTorque += totalTorque * stanceHipToSwingHipRatio * rootStrength;
    swingHipTorque += totalTorque * (1 - stanceHipToSwingHipRatio) * rootStrength;


    // get the orientation of stance hip's child
    Quaternion qStanceHip = character->getJoint(stanceHipIndex)->getChild()->getOrientation();

    // transform the torque to child's coordinates
    stanceHipTorque = qStanceHip.getComplexConjugate().rotate(stanceHipTorque);

    // apply limit to the torque just like how it was done in the calculation of torques for other joints
    ...
    // apply same process to another hip
    ...
    // update the torques for two hips
...
void SimBiController::computeHipTorques(const Quaternion& qRootD, const Quaternion& qSwingHipD, double stanceHipToSwingHipRatio){
    ...
    // loop through joints to find the swing hip and stance hip
	for (int i = 0; i < jointCount; i++)
		if (character->getJoint(i)->getParent() == root)
	...
	// now, based on the ratio of the forces the feet exert on the ground, and the ratio of the forces between the two feet, 
    // we will compute the forces that need to be applied
